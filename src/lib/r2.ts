...
export function buildObjectKey(originalName: string, postId: string) {
  const prefix = process.env.ENVIRONMENT === 'development' ? 'dev/' : '';
  return `${prefix}photos/${postId}/${originalName}`;
}
...
// Public URL strategy: adjust if using custom domain or private bucket
