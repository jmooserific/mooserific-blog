...
- **Revisited (2026-06).** The contained widening read as "just a wider white box," so it was superseded by a **full-bleed card hero**: the photo now runs edge-to-edge within the card (no inset, no radius of its own), bleeds to the full viewport width below `sm`, and settles in with a subtle CSS-only scroll reveal (`view()` timeline, reduced-motion-gated). The per-post width split was then dropped too — narrow non-hero cards next to wide hero cards read as a bug, not emphasis — so **all** post cards (and the site header) now share `max-w-6xl`, and the hero is distinguished purely by its bleed. Details in [`design-system.md`](./design-system.md) *Post card layout → Hero*.
