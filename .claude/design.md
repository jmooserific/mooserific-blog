...
Chrome is deliberately understated. Buttons and menu items share a single quiet **ghost** treatment — no fills, accent text, a subtle tint on hover. Because that same umber also colors the static meta you read (the date, the author, the footer), a ghost control needs to announce "you can click this" without a second color and without relying on hover or a cursor, which don't exist on touch. It does so by **shape**: an interactive umber element is a control with a faint resting outline (icon buttons) or an underline (text links, echoing the blue caption links), while static umber meta is plain text. The one exception is the main submit button on a form (publishing a post, signing in), which gets a filled accent treatment because on those task-focused screens the photos aren't the point. Everything else stays ghost so the photo grid remains the focal point.
